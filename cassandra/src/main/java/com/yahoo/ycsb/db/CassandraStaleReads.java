...
public class CassandraStaleReads extends DB
{
  ...
  //updation
  public static Hashtable<String,Long> cons_check = new Hashtable<String, Long>();
      ...
      try
      {
        ...
        for (ColumnOrSuperColumn oneresult : results)
        {
          ...
          column = oneresult.column;
            ...
            //updation
            String colname = key+name;
            //String colval = value.toString();
            long matchval = timestamp;
            
            if(cons_check.containsKey(colname))
                matchval = cons_check.get(colname);
  ...
  public int insert(String table, String key, HashMap<String, ByteIterator> values)
  {
      ...
      String keycolname;
      int operations = 0;
      Stack<String> insertToCons_check_key = new Stack<String>();
      Stack<Long> insertToCons_check_value = new Stack<Long>();
    if (!_table.equals(table)) {
      try
      {
        client.set_keyspace(table);
    ...
    for (int i = 0; i < OperationRetries; i++)
    {
      ...
      try
      {
        ...
        for (Map.Entry<String, ByteIterator> entry : values.entrySet())
        {
            ...
            String colval = entry.getValue().toString();
            
                        
          col = new Column();
          col.setName(ByteBuffer.wrap(colname.getBytes("UTF-8")));
          col.setValue(ByteBuffer.wrap(colval.getBytes("UTF-8")));
          col.setTimestamp(System.nanoTime());
          
          //updation
          insertTime = col.getTimestamp();
          
          //System.out.println("~~ Put for colname :"+(key+colname)+ " at timestamp "+insertTime);
          insertToCons_check_key.push(key+colname);
          insertToCons_check_value.push(insertTime);
          operations++;
        ...
        while(operations > 0){
          //System.out.println("~~ Put for colname :"+(key+colname)+ " at timestamp "+insertTime);
          cons_check.put(insertToCons_check_key.pop(), insertToCons_check_value.pop());
          operations--;
        }
        
        insertToCons_check_key.clear();
        insertToCons_check_value.clear();
        mutations.clear();
        mutationMap.clear();
        record.clear();
